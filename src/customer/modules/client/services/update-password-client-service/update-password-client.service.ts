...
class UpdatePasswordClientService {
  async execute(
    updatePasswordClientDto: IUpdatePasswordClientDto
  ): Promise<ServiceResponse<boolean>> {
    try {
      // Verificando se o usuário existe
      const user = await Client.findOne({
        where: { id: updatePasswordClientDto.id, active: true },
      });

      if (!user) throw new ApiError('Usuário não encontrado');

      // Verificando se a senha fornecida é igual salva
      if (!user.comparePassword(updatePasswordClientDto.currentPassword))
